...
const presets = [...document.querySelectorAll(".preset:not(.disabled)")];
...
function tuneTo(index) {
  const wasPlaying = !audio.paused;
  tAngle = [-28, 8, 30][((index % playlist.length) + playlist.length) % playlist.length];
  updateTuningVisual();
...
tuningKnob.addEventListener("pointerup", e => {
  ...
  if (tuningMoved) {
    const station = tAngle < -10 ? 0 : (tAngle < 20 ? 1 : 2);
    tuneTo(station);
  } else {
    tuneTo(trackIndex + 1);
  }
...
function renderMenuSection(section){
  ...
  if(section === "radio"){
    ...
    menuContent.innerHTML = `<div class="archive-list">${playlist.map((t,i)=>`<button class="archive-track" data-track="${i}"><em>0${i+1}</em><strong>${t.title}</strong><span>${t.freq} FM</span></button>`).join("")}</div>`;
